...
public class GameGUIComponent extends Canvas implements WindowController {
    ...
    private KeyboardMouseUserInputListener inputListener;
    ...
    public void runSingleGame() {
        timescale = 1;
        inputListener = new KeyboardMouseUserInputListener(messages());
        ...
        gameManager.initializeGame(
                new ImageReader(messages()),
                new SoundReader(messages()), inputListener,
                this
        );
    ...
    public boolean openYesNoDialog(String msg) {
        return messages().openYesNoDialog(msg);
    }
    ...
    private void initLoadScreen() {
        ImageRenderable loadScreenImage =
                new ImageReader(messages()).readImage(
        ...
        if(millisToSleep == minIdleTimeInMillis) {
            ...
            if(fpsHits == FPS_HITS_BEFORE_WARNING_USER) {
                messages().showMessage(
                    "Warning: your frames are taking " +
